...
impl MouseHandler {
    ...
    pub fn handle_mouse_event(
        ...
        match event.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if let Some(pane_id) = self.get_pane_border_at(event.column, event.row, pane_layout) {
                    self.start_drag(event.column, event.row, pane_id.clone());
                    return Ok(Some(MouseAction::StartResize { pane: pane_id }));
                }
    ...
    fn get_pane_border_at(&self, x: u16, y: u16, layout: &crate::protocol::LayoutInfo) -> Option<PaneId> {
        for pane in &layout.panes {
            // Check if on right border (for horizontal resizing)
            if x == pane.x + pane.width - 1 && y >= pane.y && y < pane.y + pane.height {
                return Some(pane.id.clone());
            }
            // Check if on bottom border (for vertical resizing)
            if y == pane.y + pane.height - 1 && x >= pane.x && x < pane.x + pane.width {
                return Some(pane.id.clone());
            }
        }
        None
    ...
    fn start_drag(&mut self, x: u16, y: u16, pane: PaneId) {
        self.dragging = true;
...
pub enum MouseAction {
    ...
    StartResize { pane: PaneId },
    ResizePanes { x: u16, y: u16 },
    EndResize,
