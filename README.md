...
> **⚠️ Alpha Release (v0.20.3)**: Ferrix is feature-complete with tmux/screen parity, but still in alpha testing. It includes multi-client session support, session persistence with buffer restoration, automatic session cleanup, and polished UX features including contextual help, enhanced mouse support, and intelligent error messages. Crash recovery is available via `--recover` flag (experimental). **Not recommended for production use yet** - see [Known Limitations](#known-limitations) below.
